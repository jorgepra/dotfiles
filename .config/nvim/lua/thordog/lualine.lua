...
if not status_theme_ok then
  vim.notify("lualine theme not found")
  return
end
]]
...
vim.api.nvim_set_hl(0, "WinBarContext", { fg = green, bg = "NONE" })
...
local mode = {
	...
	function()
		...
		return "  "
		-- return "  "
		-- return hl_str(" ", "SLSep") .. hl_str(" ", "SLLocation") --.. hl_str(" ", "SLSep")
	end,
	color = function()
...
local branch = {
	...
	fmt = function(str)
		local strl = string.gsub(str, "%s+", "")
		if strl == "" or strl == nil then
			strl = "!=vcs"
		end
		return hl_str("", "SLSep")
			.. hl_str("%#SLGitIcon#" .. "  ", "SLIndent")
			.. hl_str("%#SLBranchName#" .. strl .. " ", "SLProgress")
			.. hl_str(" ", "SLSep")
	end,
}
...
local current_signature = {
	function()
		...
		if not require("thordog.functions").isempty(hint) then
...
local function get_location()
	...
	if navic_location ~= nil then
		return "%#WinBarContext#" .. " " .. navic_location
	end
	return ""
end
...
lualine.setup({
	...
	sections = {
		lualine_a = { mode, branch }, -- cambiar aqui con funcion sl_hl
		lualine_b = { left_pad_alt, diagnostics, right_pad_alt },
		--lualine_c = { current_signature },
		lualine_c = { current_signature, { get_location, cond = navic.is_available } },
